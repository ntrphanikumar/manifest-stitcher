...
class HLS {
    ...
    static addSessionId(playlistObj, sessionId) {
        let tempUri = "";
        for (let i = 0; i < playlistObj.variants.length; i++) {
            let variantTemp = playlistObj.variants[i].uri;
            if (variantTemp.indexOf("?") > -1) {
                playlistObj.variants[i].uri += "&sessionId=" + sessionId;
            }
            else{
                playlistObj.variants[i].uri += "?sessionId=" + sessionId;
            }
            tempUri = playlistObj.variants[i].uri;
        }

        return !0;
    }
    static passQueryParams(playlistObj, queryParams) {

        if(!playlistObj.isMasterPlaylist && queryParams.length > 0){

            for (let i = 0; i < playlistObj.segments.length; i++) {
                let segmentTemp = playlistObj.segments[i].uri;
                if (segmentTemp.indexOf("?") > -1) {
                    playlistObj.segments[i].uri += "&" + queryParams;
                }
                else{
                    playlistObj.segments[i].uri += "?" + queryParams;
                }
                logger.log(playlistObj.segments[i].uri)
            }
        }
        if(playlistObj.isMasterPlaylist && queryParams.length > 0){

            for (let i = 0; i < playlistObj.variants.length; i++) {
                let segmentTemp = playlistObj.variants[i].uri;
                if (segmentTemp.indexOf("?") > -1) {
                    playlistObj.variants[i].uri += "&" + queryParams;
                }
                else{
                    playlistObj.variants[i].uri += "?" + queryParams;
                }
